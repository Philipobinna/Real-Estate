...
export const signin = async (req, res, next) => {
  ...
  try {
    ...
    res
      ...
      .status(200)
      .json({ msg: "Login successfully", rest });
  } catch (error) {
    next(error);
  }
};
...
try {
  ...
  if (user) {
    ...
    const newUser = new User({username: req.body.name.split(" ").join('') + Math.random().toString(36).slice(-8).toUpperCase() + "@", email: req.body.email, password: hashedPassword, avatar: req.body.photo});
    await newUser.save();
